Attribute VB_Name = "ClientUpdater"
Option Explicit
Public Sub ClientUpdater()
...
190    fileURL = "webaddress" ''put the url here
...
230    If http.Status = 200 Then
        ' grab response text as file content
...
380 On Error GoTo errorhandler
        ...
        ' sets code property,alerts if download wasnt succesful
...
480             For Each modName In ThisWorkbook.VBProject.VBComponents ' looks for module with matching name
490                 If LCase(fileObj.Name) = LCase(modName.Name) Then  ' deletes if found
...
580    Dim fNum As Integer 'then replaces,
590     fNum = FreeFile
600        Open tempFile For Output As #fNum  'or just adds if non existant
